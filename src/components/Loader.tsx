...
/* ─── Bezier curve definitions (optimized for mobile perf) ─── */
const CURVES = [
  {
...
const Loader: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  /* ─── Typing animation ─── */
  const typeText = async (text: string, delay: number = 40) => {
    setIsTyping(true);
    setDisplayText("");
  ...
  const eraseText = async (delay: number = 25) => {
  ...
  /* Progress timer — faster on mobile */
  useEffect(() => {
    if (progress >= 100) {
      const timeout = setTimeout(() => setShowButton(true), 200);
      return () => clearTimeout(timeout);
    }

    const timer = setTimeout(() => {
      setProgress((prev) => {
        const increment = Math.random() * 65 + 5;
  ...
  return (
    ...
    >
      ...
      {/* ── SVG Bezier Curves (no blur filters — GPU friendly) ── */}
      <motion.svg
        ...
        initial={{ opacity: 0 }}
        animate={{ opacity: isExiting ? 0 : 1 }}
        transition={{ duration: isExiting ? 0.4 : 0.8, ease: "easeOut" }}
      >
        <defs>
      ...
      <div className="relative z-20 flex flex-col items-center">
        ...
        >
          <div className="relative w-[240px] h-[240px] flex items-center justify-center">
            {/* Progress ring (lightweight — no blur filter) */}
            <svg
